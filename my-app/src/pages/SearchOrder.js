...
const SearchOrder = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(`/order/find/${searchQuery}`);
      ...
      setOrder(null);
      setError('Sorry, your order ID is not found, try to check your spelling. Contact us if you have any questions.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className={`${styles.paddingX} ${styles.flexCenter}`}>
      <div className={`${styles.boxWidth}`}>
        <Navbar />
        <div className="mt-8">
          <h1 className="font-poppins mt-5 text-[35px] font-semibold text-gray-900 text-center">Search Order</h1>
          <p className="font-poppins mt-5 text-gray-900 text-center">Find your order information based on order ID.</p>
          {/* Search Bar */}
          <div className="flex justify-center mt-6">
            <input
              ...
              className="p-2 border font-poppins rounded-l-lg w-64"
            />
            <button
              onClick={handleSearch}
              className="p-2 bg-cyan-300 hover:bg-cyan-400 text-black font-poppins  rounded-r-lg"
            >
              Search
            </button>
          </div>
          ...
          {order && (
            <div className="mt-6 border-t border-gray-100">
              <div className="py-6 border-[3px] mt-5 rounded-[20px] border-gray-200 flex flex-col md:flex-row">
                
                {/* Left Column: Order and User Information */}
                <div className="flex-1 p-5 ml-10 font-poppins">
                ...
                <div className="flex-1 p-5 flex flex-col items-center ">
                  {order.proofOfTransfer && order.proofOfTransfer.length > 0 ? (
                    ...
                    <div>
                      <p className="font-poppins">Proof of payment has not been provided yet, login with your account to upload your proof of payment.</p>
                    </div>
                  )}
                </div>
